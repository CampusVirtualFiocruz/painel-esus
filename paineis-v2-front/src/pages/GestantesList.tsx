...
export function tipoResidencia(tipo: string) {
    if (tipo === 'Urbano') {
        return <div className="iconCircle iconUrbano ms-2" title="Urbano">U</div>
    } else if (tipo === 'Rural') {
        return <div className="iconCircle iconRural ms-2" title="Rural">R</div>
    } else {
        return ''
    }
}
...
export function temPendencias(tipo: boolean) {
    if (!tipo) {
        return <AiFillExclamationCircle size={'1.2rem'} className="aviso" title="Pendências" />
    }
    ...
    return (
        <div id="page-painel">
            ...
            <div className="contentWrapper">
                            ...
                            <div className="table-responsive-md">
                                <table className="table table-striped table-bordered">
                                    <thead>
                                        <tr style={{ backgroundColor: "#EEEEEE" }}>
                                            ...
                                            <th scope="col" className="text-center align-middle">Idade</th>
                                            <th scope="col" className="text-center">IG 1ª Consulta</th>
                                            <th scope="col" className="text-center">Total de Consultas</th>
                                            <th scope="col" className="text-center">Tem Diabetes</th>
                                            <th scope="col" className="text-center">Tem Has</th>
                                            <th scope="col" className="text-center">Exame VDRL e Anti HIV</th>
                                            <th scope="col" className="text-center">Consulta Odontológica</th>
                                            <th scope="col" className="text-center">Esquema Vacinal</th>
                                        </tr>
                                    </thead>
                                    <tbody className="tbody-gestantes">
                                        {data?.gestantes.map((gestante: TGestante, i: number) => {
                                            return (
                                                <tr key={i}>
                                                    <th onClick={() => handleToGestante(gestante)} className="d-flex align-items-center justify-content-between">
                                                        <span className="nomeGestante">{gestante.nome ?? 'NÃO CADASTRADO'}</span>
                                                        <div className="d-flex">
                                                            {temPendencias(gestante.pendencia)}
                                                            {tipoResidencia(gestante.tipoResidencia)}
                                                        </div>
                                                    </th>
                                                    <td className="text-center">{gestante.idade ?? 'NÃO CADASTRADO'}</td>
                                                    <td className="text-center">{gestante.igPrimeiraConsulta ?? 0}</td>
                                                    <td className="text-center">{gestante.totalConsultas ?? 0}</td>
                                                    <td className="text-center">{gestante.temDiabetes ?? 'NÃO CADASTRADO'}</td>
                                                    <td className="text-center">{gestante.temHas ?? 'NÃO CADASTRADO'}</td>
                                                    <td className="text-center">{gestante.exameVdrlAntiHiv ?? 'NÃO CADASTRADO'}</td>
                                                    <td className="text-center">{gestante.consultaOdonto ?? 'NÃO CADASTRADO'}</td>
                                                    <td className="text-center">{gestante.esquemaVacinal ?? 'NÃO CADASTRADO'}</td>
                                                </tr>
                                            )
                                        })}
                                    </tbody>
                                ...
                                <div className="mb-2">
                                    <p className="text-end mb-0 small">
                                        <AiFillExclamationCircle size={'1.3rem'} className="mb-1 me-2 aviso" />Pendências: Socilitação de exame, avaliação de exames ou esquema vacina incompleto.
                                    </p>
                                </div>
                            </div>
                ...
                <div className="d-flex flex-column containerButtons align-items-center">
                    {!isLoading &&
                        <button
                            type="button"
                            onClick={() => { }}
