...
type AccuteStackMap = { [key: string]: {
    [key: string]: number,
    ...
    const result = {
        ...
        stack: [
            new FebreExantematica(
                Object.values(dateMap).map( (item:any) => item.totalFebreExantematicas ) as [number]
            ),
            new FebreInespecifica(
                Object.values(dateMap).map( (item:any) => item.totalFebreInespecificas ) as [number]
            ),
            new InfeccaoRespiratoria(
                Object.values(dateMap).map( (item:any) => item.totalInfeccoesRespiratorias ) as [number]
            ),
            new InfeccaoIntestinal(
                Object.values(dateMap).map( (item:any) => item.totalInfeccoesIntestinais ) as [number]
            ),
            new OutrosAtendimentos(
                Object.values(dateMap).map( (item:any) => item.totalAtendimentosPorOutrosCasos ) as [number]
            ),
            new AtendimentosAgudos(
                Object.values(dateMap).map( (item:any) => (
