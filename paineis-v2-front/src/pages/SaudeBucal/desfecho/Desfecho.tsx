...
const Desfecho = () => {
    ...
    const { data: desfechoResponse, isLoading, error } = useQuery(
    ...
    return (<AsyncDataLoad {...{ isLoading, error }}>
        <div className="col-12">
            <h2 >Saúde Bucal por desfecho de atendimento</h2>
            {!isLoading && desfechoResponse && Array.from(desfechoResponse).map((item: DesfechResponse, i: number) => (
                <div key={i} className="d-flex align-items-center mt-2">
                    <div className="desfecho d-flex">
                        ...
                        <div className="w30 d-flex hprogess">
                            <Progress value={item.percent} className="w-75 hprogess" barStyle={{ backgroundColor: random_hex_color_code() }} />
                        </div>
                    </div>
