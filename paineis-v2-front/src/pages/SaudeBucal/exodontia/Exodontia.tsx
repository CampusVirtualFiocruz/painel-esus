...
const ExodontiaPie = (props: ExodontiaResponse[]) => {
    ...
    const data = props?.map((i: ExodontiaResponse) => ({ value: i.value, name: i.label }))
    const options = {
        color: ['#09406a', '#78b4d0', '#78a4d0', '#e4e4e4'],
        tooltip: {
            trigger: "item",
            formatter: "{b0}: {c0}({d}%)"
        },
...
const Exodontia = () => {
    ...
    const { data: exodontiaResponse, isLoading, error } = useQuery(
    ...
    return (<AsyncDataLoad {...{ isLoading, error }}>
        ...
        <div className="col-12 dflex">
            {!isLoading && exodontiaResponse && <ExodontiaPie {...exodontiaResponse} />}
        </div>
