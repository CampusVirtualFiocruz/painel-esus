...
const useReportDataIdosas = ({ ubsId, squadId }: reportBasicInfo) => {
  ...
  const totalAtendidasQuery = useQuery(
    ...
    async () => {
      const response = await Api.get(
        `/elderly/total-medical-cares${ubsParam}`,
        defaultParam
      );
      return response?.data?.data || response.data;
    },
  ...
  const errors = {
    ...
    'duas-consultas-medicas-enfermagem':
      duasConsultasMedicasEnfermagemQuery.error,
    'dois-registros-peso-altura': doisRegistrosPesoAlturaQuery.error,
    'duas-visitas-domiciliares-acs-tacs':
      duasVisitasDomiciliaresAcsTacsQuery.error,
  ...
  const data = {
    ...
    'duas-consultas-medicas-enfermagem': {
      data: duasConsultasMedicasEnfermagemQuery.data,
    },
    'dois-registros-peso-altura': { data: doisRegistrosPesoAlturaQuery.data },
    'duas-visitas-domiciliares-acs-tacs': {
      data: duasVisitasDomiciliaresAcsTacsQuery.data,
    },
