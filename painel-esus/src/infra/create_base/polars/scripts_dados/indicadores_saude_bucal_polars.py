...
def gerar_banco():
    ...
    tb_pessoa = tb_pessoa.with_columns(
        pl.when(pl.col("tp_identidade_genero_cidadao") == "HOMEM_CIS")
        .then(pl.lit("Homem cisgênero"))
        .when(pl.col("tp_identidade_genero_cidadao") == "MULHER_CIS")
        .then(pl.lit("Mulher cisgênero"))
        .when(pl.col("tp_identidade_genero_cidadao") == "HOMEM_TRANS")
        .then(pl.lit("Homem transgênero"))
        .when(pl.col("tp_identidade_genero_cidadao") == "MULHER_TRANS")
        .then(pl.lit("Mulher transgênero"))
        .when(pl.col("tp_identidade_genero_cidadao") == "TRAVESTI")
        .then(pl.lit("Travesti"))
        .when(pl.col("tp_identidade_genero_cidadao") == "NAO_BINARIO")
        .then(pl.lit("Não-binário"))
        .when(pl.col("tp_identidade_genero_cidadao") == "OUTRO_IDENTIDADE_GENERO")
        .then(pl.lit("Outra"))
        .otherwise(pl.lit(None))
    ...
    fao_vars = (
        fao_v2.filter(pl.col("dt_atendimento") >= dt_30meses)
        .select(
            "co_fat_cidadao_pec",
            "st_paciente_necessidades_espec",
            "st_gestante",
        ...
        .agg(
            pl.col("st_paciente_necessidades_espec")
            .first()
    ...
    ).unique(subset="co_fat_cidadao_pec", keep="first")
    ...
    tb_pessoa_v2 = (
        ...
        .with_columns(
            ...
            .then(pl.format("{} dias", pl.col("idade_dias")))
            .when(pl.col("idade_anos_completos") < 1)  # Menor de 1 ano completo
            .then(pl.format("{} meses", pl.col("idade_meses_completos")))
    ...
    tb_pessoa_odonto = (
        tb_pessoa_v2.join(
            ...
            pl.when(
                (pl.col("dt_ultimo_atendimento") >= dt_30meses)
                & pl.col("dt_ultimo_atendimento").is_not_null()
            )
            .then(1)
            .otherwise(0)
            .cast(pl.Int8)
            .alias("atendimento_odonto"),
            pl.when(
                (pl.col("dt_ultima_fci") >= dt_30meses)
                & pl.col("dt_ultima_fci").is_not_null()
            )
            .then(1)
            .otherwise(0)
            .cast(pl.Int8)
            .alias("cadastradas_odonto"),
        )
    ...
    tb_pessoa_odonto_populacao = tb_pessoa_odonto.select(
        "co_fat_cidadao_pec", "cadastradas_odonto", "atendimento_odonto"
    )

    fao_v3 = (
        fao_v2.join(tb_pessoa_odonto_populacao, on="co_fat_cidadao_pec", how="left")
    ).with_columns(
    ...
    atendidas = (
        tb_pessoa_odonto.filter(pl.col("atendimento_odonto") == 1)
    ...
    tb_pessoa_odonto = tb_pessoa_odonto.with_columns(
        pl.when(
            (pl.col("idade_anos_completos") >= 0)
            & (pl.col("idade_anos_completos") <= 1)
        )
        .then(pl.lit("0a1"))
        .when(
            (pl.col("idade_anos_completos") >= 1)
            & (pl.col("idade_anos_completos") <= 4)
        )
        .then(pl.lit("1a4"))
        .when(
            (pl.col("idade_anos_completos") >= 5)
            & (pl.col("idade_anos_completos") <= 9)
        )
        .then(pl.lit("5a9"))
        .when(
            (pl.col("idade_anos_completos") >= 10)
            & (pl.col("idade_anos_completos") <= 14)
        )
        .then(pl.lit("10a14"))
        .when(
            (pl.col("idade_anos_completos") >= 15)
            & (pl.col("idade_anos_completos") <= 19)
        )
        .then(pl.lit("15a19"))
        .when(
            (pl.col("idade_anos_completos") >= 20)
            & (pl.col("idade_anos_completos") <= 29)
        )
        .then(pl.lit("20a29"))
        .when(
            (pl.col("idade_anos_completos") >= 30)
            & (pl.col("idade_anos_completos") <= 39)
        )
        .then(pl.lit("30a39"))
        .when(
            (pl.col("idade_anos_completos") >= 40)
            & (pl.col("idade_anos_completos") <= 49)
        )
        .then(pl.lit("40a49"))
        .when(
            (pl.col("idade_anos_completos") >= 50)
            & (pl.col("idade_anos_completos") <= 59)
        )
        .then(pl.lit("50a59"))
        .when(
            (pl.col("idade_anos_completos") >= 60)
            & (pl.col("idade_anos_completos") <= 69)
        )
        .then(pl.lit("60a69"))
        .when(
            (pl.col("idade_anos_completos") >= 70)
            & (pl.col("idade_anos_completos") <= 79)
        )
        .then(pl.lit("70a79"))
        .when(pl.col("idade_anos_completos") >= 80)
        .then(pl.lit("80mais"))
    ...
    indicador_primeira_consulta_at = (
        ...
        .group_by("co_fat_cidadao_pec")
        .agg(pl.col("dt_atendimento").max().alias("dt_primeira_consulta_programatica"))
        .with_columns(
    ...
    atendidas = atendidas.join(
        indicador_primeira_consulta_at, on="co_fat_cidadao_pec", how="left"
    ).with_columns(
    ...
    tratamento_odontologico_concluido_at = (
        ...
        .group_by("co_fat_cidadao_pec")
        .agg(pl.col("dt_atendimento").max().alias("dt_tratamento_concluido_atendidas"))
        .with_columns(
            pl.when((pl.col("dt_tratamento_concluido_atendidas") >= dt_24meses))
            .then(1)
            .otherwise(0)
            .cast(pl.Int8)
    ...
    atendidas = atendidas.join(
        tratamento_odontologico_concluido_at, on="co_fat_cidadao_pec", how="left"
    ).with_columns(pl.col("agg_tratamento_odonto_concluido_atendidas").fill_null(0))
    ...
    tb_pessoa_odonto_exodontia_at = (
        fao_v3.filter(
            ...
            & (pl.col("dt_atendimento") >= dt_30meses)
            & (
                pl.col("ds_filtro_procedimentos").str.contains(
                    r"\|0414020138|0414020146\|"
                )
            )
        )
        .with_columns(
        ...
        .agg(
            ...
            pl.col("total_exodontia_atendidas_30")
            .sum()
            .alias("total_exodontia_atendidas"),
            pl.max("agg_realizaram_exodontia_atendidas_aux").alias(
    ...
    atendidas = atendidas.join(
        tb_pessoa_odonto_exodontia_at, on="co_fat_cidadao_pec", how="left"
    ).with_columns(
    ...
    procedimentos_preventivos_regex = (
        r"\|(" + "|".join(dict_procedimentos.keys()) + r")\|"
    )

    def extrair_procedimentos(
        texto: str, substituir_por_descricao: bool = False
    ) -> str | None:
    ...
    indicador_procedimentos_preventivos_at = (
        ...
        .with_columns(
            ...
            .cast(pl.Int8)
            .alias("agg_procedimentos_preventivos_atendidas")
        )
        .group_by("co_fat_cidadao_pec")
        .agg(
            pl.max("agg_procedimentos_preventivos_atendidas").alias(
                "agg_procedimentos_preventivos_atendidas"
            ),
            pl.col("ds_filtro_procedimentos")
            .str.concat("|")
            .alias("todos_procedimentos"),
        )
        .with_columns(
    ...
    atendidas = atendidas.join(
        indicador_procedimentos_preventivos_at, on="co_fat_cidadao_pec", how="left"
    ).with_columns(
    ...
    tb_pessoa_odonto_TRA_at = (
        ...
        .with_columns(
            ...
            .alias("data_ultimo_atendimento_24"),
        )
        .group_by("co_fat_cidadao_pec")
        .agg(
        ...
        .with_columns()
        .with_columns(
            pl.when((pl.col("sum_TRA_atendidas") >= 0))
            .then(1)
            .otherwise(0)
            .cast(pl.Int8)
            .alias("agg_TRA_atendidas")
        )
        .select(["co_fat_cidadao_pec", "agg_TRA_atendidas", "dt_TRA_atendidas"])
    )
    ...
    atendidas = atendidas.join(
        tb_pessoa_odonto_TRA_at, on="co_fat_cidadao_pec", how="left"
    ).with_columns(
    ...
    cadastradas = (
        tb_pessoa_odonto.filter(pl.col("cadastradas_odonto") == 1)
    ...
    cadastradas = cadastradas.join(
        tb_pessoa_odonto.select(
            "co_fat_cidadao_pec", "no_sexo_cidadao", "ds_raca_cor", "faixa_etaria"
        ),
        on="co_fat_cidadao_pec",
        how="left",
    )
    ...
    cadastradas = cadastradas.join(
        indicador_primeira_consulta_cad, on="co_fat_cidadao_pec", how="left"
    ).with_columns(
    ...
    tratamento_odontologico_concluido_cad = (
        ...
        .group_by("co_fat_cidadao_pec")
        .agg(
            pl.col("dt_atendimento").max().alias("dt_tratamento_concluido_cadastradas")
        )
        .with_columns(
            pl.when((pl.col("dt_tratamento_concluido_cadastradas") >= dt_24meses))
            .then(1)
            .otherwise(0)
            .cast(pl.Int8)
    ...
    cadastradas = cadastradas.join(
        tratamento_odontologico_concluido_cad, on="co_fat_cidadao_pec", how="left"
    ).with_columns(
    ...
    tb_pessoa_odonto_exodontia_cad = (
        ...
        .with_columns(
            pl.when(pl.col("ds_filtro_procedimentos").str.contains(regex_pattern))
            .then(1)
            .otherwise(0)
        ...
        .group_by("co_fat_cidadao_pec")
        .agg(
            pl.col("total_exodontia_cadastradas_30")
            .sum()
            .alias("total_exodontia_cadastradas"),
            pl.col("total_exodontia_cadastradas_24")
            .sum()
            .alias("sum_total_exodontia_cadastradas_24"),
        )
        .with_columns(
            pl.when((pl.col("sum_total_exodontia_cadastradas_24") > 0))
            .then(1)
            .otherwise(0)
            .cast(pl.Int8)
    ...
    cadastradas = cadastradas.join(
        tb_pessoa_odonto_exodontia_cad, on="co_fat_cidadao_pec", how="left"
    ).with_columns(
    ...
    indicador_procedimentos_preventivos_cad = (
        ...
        .with_columns(
            ((pl.col("dt_atendimento") >= dt_24meses))
            .cast(pl.Int8)
            .alias("agg_procedimentos_preventivos_cadastradas")
        )
        .group_by("co_fat_cidadao_pec")
        .agg(
            pl.max("agg_procedimentos_preventivos_cadastradas").alias(
    ...
    cadastradas = cadastradas.join(
        indicador_procedimentos_preventivos_cad, on="co_fat_cidadao_pec", how="left"
    ).with_columns(
    ...
    tb_pessoa_odonto_TRA_cad = (
        ...
        .with_columns(
            pl.when((pl.col("sum_TRA_cad") >= 0))
            .then(1)
            .otherwise(0)
            .cast(pl.Int8)
            .alias("agg_TRA_cadastradas")
        )
        .select(["co_fat_cidadao_pec", "agg_TRA_cadastradas", "dt_TRA_cadastradas"])
    )
    ...
    cadastradas = cadastradas.join(
        tb_pessoa_odonto_TRA_cad, on="co_fat_cidadao_pec", how="left"
    ).with_columns(
    ...
    tb_pessoa_odonto_final = (
        tb_pessoa_odonto_final.join(cadastradas, on="co_fat_cidadao_pec", how="left")
        .join(atendidas, on="co_fat_cidadao_pec", how="left")
        .with_columns(
            pl.col("agg_primeira_consulta_atendidas").fill_null(0),
