...
def gerar_banco():
    ...
    tb_pessoa_odonto = (tb_pessoa_odonto
        .with_columns(
        pl.when((pl.col("idade") >= 0) & (pl.col("idade") <= 1)).then(pl.lit("0a1"))
        .when((pl.col("idade") >= 1) & (pl.col("idade") <= 4)).then(pl.lit("1a4"))
    ...
    def extrair_procedimentos(texto: str, substituir_por_descricao: bool = False) -> str | None:
        ...
        if substituir_por_descricao:
            return "|".join([dict_procedimentos.get(codigo, codigo) for codigo in codigos])
        return "|".join(codigos)
