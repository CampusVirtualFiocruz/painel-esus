...
def gerar_banco():
    ...
    # FAI
    selected_columns_atd_ind = ["co_fat_cidadao_pec", "co_seq_fat_atd_ind","nu_altura","nu_peso","co_dim_tempo","co_dim_cbo_1", "co_dim_cbo_2", "dt_nascimento", "ds_filtro_proced_avaliados","ds_filtro_ciaps"]

    # FAOI
    columns_fao = ['co_fat_cidadao_pec','co_dim_tempo',"nu_altura","nu_peso",'co_dim_cbo_1','co_dim_cbo_2','co_seq_fat_atd_odnt','ds_filtro_procedimentos','ds_filtro_ciaps']
    ...
    tb_pessoa_v2 = (
        ...
        .with_columns([
            ...
            ((pl.col("idade_em_dias") % 365) // 30).alias("idade_meses"),
            (pl.col("idade_em_dias") % 30).alias("idade_dias")
        ])
        .with_columns(

            label_anos = pl.when(pl.col("idade_anos") == 1).then(pl.lit(" ano")).otherwise(pl.lit(" anos")),
            label_meses = pl.when(pl.col("idade_meses") == 1).then(pl.lit(" mês")).otherwise(pl.lit(" meses")),
            label_dias = pl.when(pl.col("idade_dias") == 1).then(pl.lit(" dia")).otherwise(pl.lit(" dias")),
        )
        .with_columns(
            pl.when(pl.col("idade_em_dias") < 30)
            .then(
                pl.concat_str([
                    pl.col("idade_em_dias").cast(pl.Utf8),
                    #pl.lit(" dias")
                    pl.when(pl.col("idade_em_dias") == 1).then(pl.lit(" dia")).otherwise(pl.lit(" dias"))
                ])
            )
            .when(pl.col("idade_em_dias") < 365)
            .then(
                pl.concat_str([
                    pl.col("idade_meses").cast(pl.Utf8),
                    pl.col("label_meses"), # <-- Usa o label
                    pl.lit(" e "),
                    pl.col("idade_dias").cast(pl.Utf8),
                    pl.col("label_dias") # <-- Usa o label
                ])
            )
            .otherwise(
                pl.concat_str([
                    pl.col("idade_anos").cast(pl.Utf8),
                    pl.col("label_anos"), # <-- Usa o label
                    pl.lit(" e "),
                    pl.col("idade_meses").cast(pl.Utf8),
                    pl.col("label_meses") # <-- Usa o label
                ])
            ).alias("idade_mes_ano"),
            pl.col("idade_anos").alias("idade")
        )
    )
    ...
    indicadorI = (
        ...
        .with_columns([
            ...
            .when(
                pl.col("agg_puericultura_ate_8_dias") == 99
            )
    ...
    indicador_II = (
        ...
        .with_columns([
            ...
            .then(1)
            .when(
                pl.col("agg_puericultura_9_consultas_ate_2_anos") == 99
            )
            ...
            .then(1)
            .when(
                pl.col("agg_puericultura_9_consultas_ate_2_anos_36") == 99
            )
            .then(99)  # Se for 99 → mantém 99
            .otherwise(0)  # Caso contrário → 0
    ...
    indicadorIII = (
        ...
        .agg([
            ...
            pl.col("nu_peso").sort_by("dt_atendimento", descending=True).first().alias("nu_peso_recentes"),
            pl.col("nu_altura").sort_by("dt_atendimento", descending=True).first().alias("nu_altura_recentes"),
    ...
    classificacao_nutricional = (
        ...
        .group_by("co_fat_cidadao_pec")
        .agg([
            pl.col("nu_peso").sort_by("dt_atendimento", descending=True).first().alias("nu_peso_recente"),
        ...
        .with_columns(
            ((pl.col("dt_ultima_avaliacao_peso") - pl.col("dt_nasc_cidadao")).dt.total_days() / 30.44).round(0).cast(pl.Int64).alias("idade_meses")
    ))
    ...
    tabela_crianca_final_v2 = (
        ...
        .with_columns(
            ...
            .then(
                pl.when(pl.col("idade_em_dias") < 183)
                .then(99)
                .otherwise(0)
            )
            ...
            .then(
                pl.when(pl.col("idade_em_dias") < 365)
                .then(99)
                .otherwise(0)
            )
