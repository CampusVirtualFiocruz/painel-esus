...
def gerar_banco():
    ...
    tb_pessoa = pl.read_csv(
        input_path + os.sep + "pessoas.csv", separator=";", ignore_errors=True
    )
    ...
    fat_vis_dom = pl.read_parquet(
        input_path + os.sep + "tb_fat_visita_domiciliar.parquet"
    )
    ...
    crianca = (
        tb_pessoa.filter((pl.col("idade").cast(pl.Int32) <= 2))
        .rename({"cidadao_pec": "co_fat_cidadao_pec"})
        .with_columns(pl.col("co_fat_cidadao_pec").cast(pl.Int64))
        .select("co_fat_cidadao_pec")
    )

    crianca_fai = fai.join(
        crianca.select("co_fat_cidadao_pec"), on="co_fat_cidadao_pec", how="inner"
    )

    indicador_i_e_ii_iii_v = (
        crianca_fai.select(
            [
                "co_fat_cidadao_pec",
                "co_dim_tempo",
        ...
        .with_columns(
            (pl.col("dt_atendimento") - pl.col("dt_nascimento"))
            .dt.total_days()
            .alias("diff_dias"),
            pl.when(
    ...
    indicador_i = (
        indicador_i_e_ii_iii_v.group_by("co_fat_cidadao_pec")
        .agg(
            [
                ...
                pl.col("atd_8dias")
                .sum()
                .alias("atendimentos_medicos_enfermeiros_8d_vida"),
            ]
        )
        .drop("dt_atendimento")
    )

    indicador_i_i_iii = indicador_i_e_ii_iii_v.filter(pl.col("diff_dias") <= 729)

    indicador_i_i = (
        indicador_i_i_iii.group_by("co_fat_cidadao_pec")
        .agg(
            [
                # pl.len().alias("num_atendimentos"),
                pl.col("atd_med_enf")
                .sum()
                .alias("atendimentos_medicos_enfermeiros_puericult"),
                pl.col("dt_atendimento")
    ...
    indicador_i_i_i = (
        ...
        .agg(
            [
                # pl.len().alias("num_atendimentos"),
                pl.col("medicoes_peso_altura")
                .sum()
                .alias("medicoes_peso_altura_ate2anos"),
                pl.col("dt_atendimento")
                .filter(
                    (pl.col("medicoes_peso_altura").sum() >= 9)
                    & (
    ...
    indicador_i_v = (
        ...
        .filter(
            (
                pl.col("co_dim_cbo").is_in(acs_crianca)
            )  # Filtrar pelos códigos de ACS Criança
            & (
                pl.col("co_fat_cidadao_pec").is_not_null()
            )  # Garantir que `co_fat_cidadao_pec` não seja nulo
        )
        ...
        .explode(
            "sorted_dt_atendimento"
        )  # Explodir as datas ordenadas para calcular as condições das visitas
        .with_columns(
            # Calcular as condições para a primeira e segunda visitas
            primeira_visita=pl.when(
                (
                    (
                        pl.col("sorted_dt_atendimento") - pl.col("dt_nascimento")
                    ).dt.total_days()
                    >= 0
                )
                & (
        ...
        .agg(
            ...
            indicador_visitas_domiciliares_acs=pl.when(
                (
                    pl.col("primeira_visita").sum() >= 1
                )  # Pelo menos duas primeiras visitas
                & (
                    pl.col("segunda_visita").sum() >= 1
                )  # Pelo menos duas segundas visitas
            )
            .then(1)
            .otherwise(0),
            # Criar a coluna `visitas_domiciliares_acs` como a soma de `primeira_visita` e `segunda_visita`
            visitas_domiciliares_acs=(
                pl.col("primeira_visita").sum() + pl.col("segunda_visita").sum()
            ).alias("visitas_domiciliares_acs"),
        )
    )

    faip_pezinho = (
        faip.filter(pl.col("tipo") == "Procedimentos Avaliados")
        .filter(pl.col("codigo").is_in(pezinho))
        .unique(
            subset=["co_seq_fat_atd_ind"]
        )  # Remove duplicates based on `co_seq_fat_atd_ind`
    )

    indicador_v = (
        indicador_i_e_ii_iii_v.select(
            "co_seq_fat_atd_ind",
            "dt_atendimento",
            "dt_nascimento",
            "co_fat_cidadao_pec",
    ...
    indicador_vac = (
        ...
        .with_columns(
            ...
            .cast(pl.Utf8)
            .str.strptime(pl.Date, "%Y%m%d")
            .alias("dt_atendimento2"),
            pl.col("dt_nascimento")
            .cast(pl.Utf8)
            .str.strptime(pl.Date, "%Y-%m-%d")
            .alias("dt_nascimento2"),
        )
        .with_columns(
            (pl.col("dt_atendimento2") - pl.col("dt_nascimento2"))
            .dt.total_days()
            .alias("diff_dias")
        )
    ...
    indicador_v_i = (
        indicador_vac.group_by("co_fat_cidadao_pec")
        .agg(
        ...
        {"n_vacina_1": "n_penta", "n_vacina_2": "n_polio", "n_vacina_3": "n__triplici"}
    )
    ...
    crianca_updated = (
        crianca.join(indicador_i, on="co_fat_cidadao_pec", how="left")
        .with_columns(
            [
                pl.col("indicador_atendimentos_medicos_enfermeiros").fill_null(2),
            ]
        )
        .join(indicador_i_i, on="co_fat_cidadao_pec", how="left")
        .with_columns(
            [
                pl.col(
                    "indicador_atendimentos_medicos_enfermeiros_puericult"
                ).fill_null(2),
            ]
        )
        .join(indicador_i_i_i, on="co_fat_cidadao_pec", how="left")
        .with_columns(
            [
                pl.col("indicador_medicoes_peso_altura_ate2anos").fill_null(2),
            ]
        )
        .join(indicador_i_v, on="co_fat_cidadao_pec", how="left")
        .with_columns(
            [
        ...
        .join(indicador_v_i, on="co_fat_cidadao_pec", how="left")
        .with_columns(
            [
                pl.col("indicador_vacinas_penta_polio_triplici").fill_null(0),
            ]
        )
        .join(indicador_v_i_i, on="co_fat_cidadao_pec", how="left")
