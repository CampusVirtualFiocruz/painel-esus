...
def ler_dados_raw(nome_parquet, columns=""):
    ...
    try:
        ...
        lazy_on = getenv(
            "LAZY_ON", False
        )  # se lazy_on true ler e escrever dados lazy, se lazy_on false, ler os dados da forma normal
...
def gerar_banco():
    ...
    crianca = tb_pessoa_v2.filter(pl.col("idade_em_dias") <= 1095)
    ...
    crianca = crianca.join(
        total_criancas_atd, on="co_fat_cidadao_pec", how="left"
    ).with_columns(
    ...
    # regex_puericultura
    regex_puericultura2 = "|".join([f"\\|{c}\\|" for c in puericultura2])
    ...
    indicadorI = (
        ...
        .with_columns(
            pl.when(
                (pl.col("ds_filtro_proced_avaliados").str.contains(regex_puericultura))
                | (pl.col("ds_filtro_ciaps").str.contains(regex_puericultura2))
            )
            .then(1)
            .otherwise(0)
            .alias("puericultura")
        ...
        .group_by("co_fat_cidadao_pec")
        .agg(pl.col("dt_atendimento").min().alias("dt_primeira_consulta"))
        ...
        .with_columns(
            ((pl.lit(today) - pl.col("dt_nasc_cidadao")).dt.total_days()).alias(
                "idade_em_dias"
            )
        )
        ...
        .with_columns(
            ...
            .then(1)
            .otherwise(pl.when(pl.col("idade_em_dias") < 8).then(99).otherwise(0))
        ...
        .with_columns(
            [
                ...
                .then(1)  # Se a primeira condição for verdadeira → 1
                .when(pl.col("agg_puericultura_ate_8_dias") == 99)
                .then(99)  # Se agg_puericultura_ate_8_dias for 99 → mantém 99
                .otherwise(0)  # Caso contrário → 0
                ...
                pl.when(
                    (pl.col("dt_primeira_consulta") >= dt_36meses)
                    & (pl.col("agg_puericultura_ate_8_dias") == 1)
                )
                .then(1)
                .when(pl.col("agg_puericultura_ate_8_dias") == 99)
    ...
    indicador_II = (
        ...
        fai_v2.join(crianca_dt_nasc, on="co_fat_cidadao_pec", how="inner")
        ...
        .with_columns(
            [
                ...
                pl.when(pl.col("dt_atendimento") >= dt_24meses)
                .then(1)
                .otherwise(0)
                .alias("consulta_ok24"),
                pl.when(pl.col("dt_atendimento") >= dt_36meses)
                .then(1)
                .otherwise(0)
                .alias("consulta_ok36"),
            ]
        ...
        .group_by("co_fat_cidadao_pec").agg(
            [
                ...
                pl.col("consulta_ok24").sum().alias("num_consultas_puericultura_24"),
                pl.col("consulta_ok36").sum().alias("num_consultas_puericultura_36"),
        ...
        .with_columns(
            pl.when(pl.col("num_consultas_puericultura_24") >= 9)
            .then(1)
            .otherwise(pl.when(pl.col("idade_em_dias") < 730).then(99).otherwise(0))
            .alias("agg_puericultura_9_consultas_ate_2_anos"),
            pl.when(pl.col("num_consultas_puericultura_36") >= 9)
            .then(1)
            .otherwise(pl.when(pl.col("idade_em_dias") < 730).then(99).otherwise(0))
            .alias("agg_puericultura_9_consultas_ate_2_anos_36"),
            pl.col("num_consultas_puericultura_36").alias("num_consultas_puericultura"),
        )
        ...
        .with_columns(
            [
                ...
                .then(1)
                .when(pl.col("agg_puericultura_9_consultas_ate_2_anos") == 99)
                .then(99)  # Se for 99 → mantém 99
                .otherwise(0)  # Caso contrário → 0
                .alias("agg_dashboard_puericultura_9_consultas_ate_2_anos"),
                pl.when(
                    (pl.col("dt_ultimo_atd") >= dt_36meses)
                    & (pl.col("agg_puericultura_9_consultas_ate_2_anos_36") == 1)
                )
                .then(1)
                .when(pl.col("agg_puericultura_9_consultas_ate_2_anos_36") == 99)
                .then(99)  # Se for 99 → mantém 99
                .otherwise(0)  # Caso contrário → 0
    ...
    registros_com_peso_altura = pl.concat(
        [fao_v2, fai_v2, vis_dom_v2], how="diagonal"
    ).filter(pl.col("nu_peso").is_not_null() & pl.col("nu_altura").is_not_null())

    # Etapa 2: Cruzar com puericultura na mesma data
    peso_altura_na_puericultura = registros_com_peso_altura.join(
        fichas_puericultura, on=["co_fat_cidadao_pec", "dt_atendimento"], how="inner"
    )

    # Etapa 3: Juntar com dados da criança
    indicadorIII = (
        peso_altura_na_puericultura.join(crianca, on="co_fat_cidadao_pec", how="inner")
        .with_columns(
            (pl.lit(today) - pl.col("dt_nasc_cidadao"))
            .dt.total_days()
            .alias("idade_em_dias"),
        ...
        .agg(
            [
                ...
                pl.col("consulta_ok24").sum().alias("num_consultas_24"),
                pl.col("consulta_ok36").sum().alias("num_consultas_36"),
                pl.col("nu_peso")
                .sort_by("dt_atendimento", descending=True)
        ...
        .with_columns(
            pl.when(pl.col("num_consultas_24") >= 9)
            .then(1)
            .when((pl.col("idade_em_dias") < 730) & (pl.col("num_consultas_24") < 9))
            .then(99)
            .otherwise(0)
            .alias("agg_9_peso_altura_24"),
            pl.when(pl.col("num_consultas_36") >= 9)
            .then(1)
            .when((pl.col("idade_em_dias") < 730) & (pl.col("num_consultas_36") < 9))
            .then(99)
            .otherwise(0)
            .alias("agg_9_peso_altura_36"),
        )
        .with_columns(
            [
                pl.when(
                    (pl.col("dt_ultima_avaliacao_peso_altura") >= dt_24meses)
                    & (pl.col("agg_9_peso_altura_24") == 1)
                )
                .then(1)
                .when(pl.col("agg_9_peso_altura_24") == 99)
                .then(99)  # Se  for 99 → mantém 99
                .otherwise(0)  # Caso contrário → 0
                .alias("agg_dashboard_9_peso_altura"),
                pl.when(
                    (pl.col("dt_ultima_avaliacao_peso_altura") >= dt_36meses)
                    & (pl.col("agg_9_peso_altura_36") == 1)
                )
                .then(1)
                .when(pl.col("agg_9_peso_altura_36") == 99)
                .then(99)  # Se  for 99 → mantém 99
                .otherwise(0)  # Caso contrário → 0
                .alias("agg_card_9_peso_altura"),
                pl.col("num_consultas_36").alias("total_peso_altura"),
            ]
        )
        .select(
            [
    ...
    indicadorIV = (
        ...
        vis_dom_v2.filter(pl.col("co_dim_cbo").is_in(acs_tacs))
        .select(["co_fat_cidadao_pec", "dt_atendimento"])
        ...
        .filter((pl.col("dias_de_vida") >= 0) & (pl.col("dias_de_vida") <= 30))
        ...
        .with_columns(
            [
                ...
                .then(1)
                .when(pl.col("agg_visita_acs_ate_30d") == 99)
                .then(99)  # Se  for 99 → mantém 99
                .otherwise(0)  # Caso contrário → 0
    ...
    indicadorV = (
        ...
        .filter((pl.col("dias_de_vida") >= 31) & (pl.col("dias_de_vida") <= 183))
        ...
        .with_columns(
            ...
            .then(1)
            .when(
                (pl.col("idade_em_dias") < 183)
                & (pl.col("total_visitas_31a183dias") == 0)
            )
            .then(99)
            .otherwise(0)
        ...
        .with_columns(
            [
                ...
                .then(1)
                .when(pl.col("agg_visita_acs_ate_6m") == 99)
                .then(99)  # Se  for 99 → mantém 99
                .otherwise(0)  # Caso contrário → 0
    ...
    indicadorVI = (
        ...
        .group_by("co_fat_cidadao_pec").agg(
            [
                ...
                .max()
                .alias("dt_ultima_consulta_odonto_ate_12m"),
                pl.col("dt_nasc_cidadao").last().alias("dt_nasc_cidadao"),
            ]
        )
        # Etapa 6: Calcular idade atual
        ...
        .with_columns(
            ...
            .when(
                ...
                & (pl.col("qtd_consultas_odont_ate_12m") == 0)
            )
            .then(99)
            .otherwise(0)
        ...
        .with_columns(
            [
                ...
                .then(1)
                .when(pl.col("agg_odonto_ate_12m") == 99)
                .then(99)  # Se  for 99 → mantém 99
                .otherwise(0)  # Caso contrário → 0
    ...
    indicadorVII = (
        ...
        .filter((pl.col("idade_em_dias") >= 365) & (pl.col("idade_em_dias") <= 730))
        ...
        .with_columns(
            [
                ...
                .then(1)
                .when(pl.col("agg_odonto_12a24m") == 99)
                .then(99)  # Se  for 99 → mantém 99
                .otherwise(0)  # Caso contrário → 0
    ...
    n_odonto_ate_24 = (
        ...
        # Etapa 4: Filtrar atendimentos ate 24 meses (0–730 dias)
        .filter((pl.col("idade_em_dias") >= 0) & (pl.col("idade_em_dias") <= 730))
        # Etapa 5: Agrupar por criança e contar atendimentos
        .group_by("co_fat_cidadao_pec")
        .agg(
            [
                pl.len().alias("num_odonto_ate24m_qtd"),
                pl.col("dt_nasc_cidadao").last().alias("dt_nasc_cidadao"),
            ]
        )
        # Etapa 6: Calcular idade atual
        ...
        .with_columns(
            ...
            .then(pl.lit(99))
            .otherwise(pl.col("num_odonto_ate24m_qtd"))
            .alias("num_odonto_ate24m")
        )
        .select(
            [
                "co_fat_cidadao_pec",
    ...
    indicadorVIII = (
        ...
        .agg(
            [
                ...
                pl.col("dt_atendimento")
                .max()
                .alias("dt_ultima_avaliacao_desenvolvimento"),
            ]
        )
    ...
    consumo_puericultura = mca_v2.join(
        fai_puericultura,
    ...
    indicadorIX = (
        consumo_puericultura.join(crianca_dt_nasc, on="co_fat_cidadao_pec", how="inner")
    ...
    classificacao_nutricional = (
        ...
        #  Juntar com a base de crianças
        .join(crianca, on="co_fat_cidadao_pec", how="inner")
        #  Agrupar e contar apenas aferições recentes de peso
        .group_by("co_fat_cidadao_pec").agg(
            [
                pl.col("nu_peso")
                .sort_by("dt_atendimento", descending=True)
                .first()
                .alias("nu_peso_recente"),
                pl.col("dt_atendimento").max().alias("dt_ultima_avaliacao_peso"),
                pl.col("dt_nasc_cidadao").last().alias("dt_nasc_cidadao"),
                pl.col("no_sexo_cidadao").last().alias("sexo"),
            ]
        )
        #  Calcular idade na data da ultima avaliacao
        .with_columns(
            (
                (
                    pl.col("dt_ultima_avaliacao_peso") - pl.col("dt_nasc_cidadao")
                ).dt.total_days()
                / 30.44
            )
            .round(0)
    ...
    classificacao_final = classificacao_final.select(
        "co_fat_cidadao_pec", "descricao_classificacao"
    )
    # print(classificacao_final.select([
    #        "co_fat_cidadao_pec",
    #        "sexo",
    #        "idade_meses",
    #        "nu_peso_recente",
    ...
    tabela_crianca_final = (
        ...
        .join(indicadorI, on="co_fat_cidadao_pec", how="left")
        .join(indicador_II, on="co_fat_cidadao_pec", how="left")
        .join(indicadorIII, on="co_fat_cidadao_pec", how="left")
        .join(indicadorIV, on="co_fat_cidadao_pec", how="left")
        .join(indicadorV, on="co_fat_cidadao_pec", how="left")
        .join(indicadorVI, on="co_fat_cidadao_pec", how="left")
        .join(indicadorVII, on="co_fat_cidadao_pec", how="left")
        .join(indicadorVIII, on="co_fat_cidadao_pec", how="left")
        .join(indicadorIX, on="co_fat_cidadao_pec", how="left")
        .join(n_odonto_ate_24, on="co_fat_cidadao_pec", how="left")
        .join(classificacao_final, on="co_fat_cidadao_pec", how="left")
    ...
    tabela_crianca_final_v2 = tabela_crianca_final.with_columns(
        ...
        .otherwise(pl.col("agg_dashboard_puericultura_ate_8_dias"))
        .alias("agg_dashboard_puericultura_ate_8_dias"),
        pl.when(pl.col("agg_card_puericultura_ate_8_dias").is_null())
        .then(pl.when(pl.col("idade_em_dias") <= 8).then(99).otherwise(0))
        .otherwise(pl.col("agg_card_puericultura_ate_8_dias"))
        .alias("agg_card_puericultura_ate_8_dias"),
        #  até 2 anos
        pl.when(pl.col("agg_dashboard_puericultura_9_consultas_ate_2_anos").is_null())
        .then(pl.when(pl.col("idade_em_dias") <= 730).then(99).otherwise(0))
        .otherwise(pl.col("agg_dashboard_puericultura_9_consultas_ate_2_anos"))
        .alias("agg_dashboard_puericultura_9_consultas_ate_2_anos"),
        pl.when(pl.col("agg_card_puericultura_9_consultas_ate_2_anos").is_null())
        .then(pl.when(pl.col("idade_em_dias") <= 730).then(99).otherwise(0))
        .otherwise(pl.col("agg_card_puericultura_9_consultas_ate_2_anos"))
        .alias("agg_card_puericultura_9_consultas_ate_2_anos"),
        # até 2 anos (peso/altura)
        pl.when(pl.col("agg_dashboard_9_peso_altura").is_null())
        .then(pl.when(pl.col("idade_em_dias") <= 730).then(99).otherwise(0))
        .otherwise(pl.col("agg_dashboard_9_peso_altura"))
        .alias("agg_dashboard_9_peso_altura"),
        pl.when(pl.col("agg_card_9_peso_altura").is_null())
        .then(pl.when(pl.col("idade_em_dias") <= 730).then(99).otherwise(0))
        .otherwise(pl.col("agg_card_9_peso_altura"))
        .alias("agg_card_9_peso_altura"),
        # até 30 dias
        pl.when(pl.col("agg_dashboard_visita_acs_ate_30d").is_null())
        .then(pl.when(pl.col("idade_em_dias") <= 30).then(99).otherwise(0))
        .otherwise(pl.col("agg_dashboard_visita_acs_ate_30d"))
        .alias("agg_dashboard_visita_acs_ate_30d"),
        pl.when(pl.col("agg_card_visita_acs_ate_30d").is_null())
        .then(pl.when(pl.col("idade_em_dias") <= 30).then(99).otherwise(0))
        .otherwise(pl.col("agg_card_visita_acs_ate_30d"))
        .alias("agg_card_visita_acs_ate_30d"),
        # até 6 meses
        pl.when(pl.col("agg_dashboard_visita_acs_ate_6m").is_null())
        .then(pl.when(pl.col("idade_em_dias") < 183).then(99).otherwise(0))
        .otherwise(pl.col("agg_dashboard_visita_acs_ate_6m"))
        .alias("agg_dashboard_visita_acs_ate_6m"),
        pl.when(pl.col("agg_card_visita_acs_ate_6m").is_null())
        .then(pl.when(pl.col("idade_em_dias") < 183).then(99).otherwise(0))
        .otherwise(pl.col("agg_card_visita_acs_ate_6m"))
        .alias("agg_card_visita_acs_ate_6m"),
        #  até 12 meses
        pl.when(pl.col("agg_dashboard_odonto_ate_12m").is_null())
        .then(pl.when(pl.col("idade_em_dias") < 365).then(99).otherwise(0))
        .otherwise(pl.col("agg_dashboard_odonto_ate_12m"))
        .alias("agg_dashboard_odonto_ate_12m"),
        pl.when(pl.col("agg_card_odonto_ate_12m").is_null())
        .then(pl.when(pl.col("idade_em_dias") < 365).then(99).otherwise(0))
        .otherwise(pl.col("agg_card_odonto_ate_12m"))
        .alias("agg_card_odonto_ate_12m"),
        pl.when(pl.col("agg_dashboard_odonto_12a24m").is_null())
        .then(pl.when(pl.col("idade_em_dias") < 730).then(99).otherwise(0))
        .otherwise(pl.col("agg_dashboard_odonto_12a24m"))
        .alias("agg_dashboard_odonto_12a24m"),
        pl.when(pl.col("agg_card_odonto_12a24m").is_null())
        .then(pl.when(pl.col("idade_em_dias") < 730).then(99).otherwise(0))
        .otherwise(pl.col("agg_card_odonto_12a24m"))
        .alias("agg_card_odonto_12a24m"),
    )
    ...
    tabela_crianca_final = tabela_crianca_final.with_columns(
        ...
        pl.col("descricao_classificacao").fill_null("Sem registro do peso"),
        # pl.col("num_odonto_ate24m").fill_null(
        #      pl.when(pl.col("idade_em_dias") < 730)
        #      .then(99)
        #      .otherwise(0)
        # )
    ...
    tabela_crianca_final = tabela_crianca_final.select(
        "agg_dashboard_puericultura_ate_8_dias",
        "agg_card_puericultura_ate_8_dias",
        "agg_dashboard_puericultura_9_consultas_ate_2_anos",
        "agg_card_puericultura_9_consultas_ate_2_anos",
