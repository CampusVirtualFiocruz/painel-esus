...
def gerar_banco():
   ...
   # fai = pl.read_parquet(caminho_pasta+"tb_fat_atendimento_individual.parquet",columns=selected_columns_atd_ind)
    ...
    idoso = (idoso
        .with_columns(
        pl.when((pl.col("idade") >= 60) & (pl.col("idade") <= 69)).then(pl.lit("60a69"))
        .when((pl.col("idade") >= 70) & (pl.col("idade") <= 79)).then(pl.lit("70a79"))
        .when((pl.col("idade") >= 80) & (pl.col("idade") <= 89)).then(pl.lit("80a89"))
        .when((pl.col("idade") >= 90) & (pl.col("idade") <= 99)).then(pl.lit("90a99"))
        .when(pl.col("idade") >= 100).then(pl.lit("100mais"))
    ...
    fai_idoso_12meses = (
        ...
        .select(
            ...
            "agg_classificacao_consulta_medica_enfermas"  # nova coluna aqui
        )
    )
    ...
    fai_idoso_12meses = (
        ...
        .select(
            ...
            "agg_classificacao_consulta_medica_enfermas"  # nova columna 160
        )
    )
    ...
    idoso = idoso.join(
        ...
        ## Nova coluna de classificação
        pl.col("agg_classificacao_consulta_medica_enfermas")
        .fill_null("Nenhuma")
        .alias("agg_classificacao_consulta_medica_enfermas")
    )
    ...
    indicador_peso_altura_v1 = (
        peso_altura
        .filter(
            (pl.col("co_dim_cbo").is_in(peso_altura_cbo)) | (pl.col("co_dim_cbo_1").is_in(peso_altura_cbo) | pl.col("co_dim_cbo_2").is_in(peso_altura_cbo))
        )
        .filter(
        ...
        .group_by("co_fat_cidadao_pec")
        .agg(
            (pl.col("dt_atendimento") >= calcular_data(24)).sum().alias("total_peso_altura_24"),
            (pl.col("dt_atendimento") >= calcular_data(12)).sum().alias("total_peso_altura_12"),
        ...
        .with_columns(
            pl.when(pl.col("total_peso_altura_24") >= 2) # Coluna indicando se há pelo menos 2 registros simultâneos
            .then(1)
            .otherwise(0)
            .alias("2_peso_altura_24"),

            pl.when(pl.col("total_peso_altura_12") >= 2) # Coluna indicando se há pelo menos 2 registros simultâneos
            .then(1)
            .otherwise(0)
    ...
    peso_altura_medico_enf = (
        peso_altura
        .select("co_fat_cidadao_pec","dt_atendimento","co_dim_cbo","co_dim_cbo_1","co_dim_cbo_2")
        .filter(
            (pl.col("co_dim_cbo_1").is_in(medicos_codigos) |
            pl.col("co_dim_cbo_2").is_in(medicos_codigos) |
            pl.col("co_dim_cbo_1").is_in(enfermeiros_codigos) |
        ...
        # pl.col("dt_atendimento").filter(pl.col("is_enfermeiro") & (pl.col("dt_atendimento")>= dt_12meses)).alias("dts_enfermeiro"),
            pl.col("dt_atendimento").filter( (pl.col("dt_atendimento")>= dt_24meses)).unique() .alias("dts_24"),
            pl.col("dt_atendimento").filter( (pl.col("dt_atendimento")>= dt_12meses)).unique() .alias("dts_12"),
        )
        .filter(
        (pl.col("dts_24").list.len() > 0 )  & ( pl.col("dts_12").list.len() > 0)
        )
    )
    ...
    indicador_idoso_peso_altura = indicador_peso_altura_v1.join(
        peso_altura_medico_enf,
        on="co_fat_cidadao_pec",
        how="left"
    ).with_columns(
        pl.col("dt_atendimentos_12").list.set_intersection("dts_12").alias("datas_match_12"),
        pl.col("dt_atendimentos_24").list.set_intersection("dts_24").alias("datas_match_24")
    ).with_columns(
    ...
    idoso = idoso.join(
        ...
        pl.when(pl.col("idade") > 60)
            ...
            #.otherwise(pl.col("agg_alerta_creatinina"))
            .otherwise(pl.col("agg_alerta_creatinina").fill_null(0))
            .alias("agg_alerta_creatinina")
    )
    ...
    visita_asc = (
        ...
        .with_columns(
            ...
            ## NOVA COLUNA: classificação em "Duas ou mais", "Uma", "Nenhuma"
            pl.when(pl.col("total_visitas_domiciliares_acs_12") >= 2)
            .then(pl.lit("Duas ou mais"))
            .when(pl.col("total_visitas_domiciliares_acs_12") == 1)
            .then(pl.lit("Uma"))
            .otherwise(pl.lit("Nenhuma"))
        ...
        .select(
            ...
            "agg_classificacao_visitas_domiciliares_acs"  # nova coluna
        )
    )
    ...
    # pl.col("agg_alerta_visitas_domiciliares_acs").fill_null(1),
        pl.when(pl.col("idade") > 60)
            .then(pl.col("agg_alerta_visitas_domiciliares_acs").fill_null(1))
            #.otherwise(pl.col("agg_alerta_visitas_domiciliares_acs"))
            .otherwise(pl.col("agg_alerta_visitas_domiciliares_acs").fill_null(0))
            .alias("agg_alerta_visitas_domiciliares_acs"),

        ## NOVA COLUNA — preencher "Nenhuma" quando nulo
        pl.col("agg_classificacao_visitas_domiciliares_acs")
        .fill_null("Nenhuma")
        .alias("agg_classificacao_visitas_domiciliares_acs")
    )
    ...
    vacina_registro = (
        ...
        .with_columns(
            ...
            pl.when(
                (pl.col("ds_filtro_imunobiologico").str.contains(r"\b(33|77)\b"))
                )
            .then(1)
            .otherwise(0)
    ...
    idoso = idoso.join(
        ...
        pl.when(pl.col("idade") > 60)
            .then(pl.col("agg_alerta_vacinas_influenza").fill_null(1))
            #.otherwise(pl.col("agg_alerta_vacinas_influenza"))
            .otherwise(pl.col("agg_alerta_vacinas_influenza").fill_null(0))
            .alias("agg_alerta_vacinas_influenza")
    )
    ...
    idoso = idoso.join(
        ...
        pl.when(pl.col("idade") > 60)
            .then(pl.col("agg_alerta_cirurgiao_dentista").fill_null(1))
            #.otherwise(pl.col("agg_alerta_cirurgiao_dentista"))
            .otherwise(pl.col("agg_alerta_cirurgiao_dentista").fill_null(0))
            .alias("agg_alerta_cirurgiao_dentista")
    )
    ...
    ivcf_aplicado = (
        ...
        .with_columns(
            pl.when(pl.col("nu_resultado") <= 6)
            .then(pl.lit("Baixo Risco de VCF"))
            .when((pl.col("nu_resultado") >= 7) & (pl.col("nu_resultado") <= 14))
            .then(pl.lit("Moderado Risco de VCF"))
            .otherwise(pl.lit("Alto Risco de VCF"))
            .alias("classificacao_risco_vcf")
        )
        .group_by("co_fat_cidadao_pec")
        .agg(
            pl.col("idade").first().alias("idade"),
            ...
            pl.col("classificacao_risco_vcf").first().alias("classificacao_risco_vcf")
    ...
    idoso = idoso.join(
        ...
        pl.when(pl.col("idade") > 60)
            .then(pl.col("agg_alerta_ivcf_aplicado").fill_null(1))
            #.otherwise(pl.col("agg_alerta_ivcf_aplicado"))
            .otherwise(pl.col("agg_alerta_ivcf_aplicado").fill_null(0))
            .alias("agg_alerta_ivcf_aplicado"),
        pl.col("classificacao_risco_vcf").alias("agg_classificacao_risco_vcf")
    )
    ...
    print(f"Tempo total de execução: {execution_time:.2f} segundos idoso")