...
def gerar_banco():
    ...
    tb_pessoa = pl.read_parquet(
        input_path + os.sep + "tb_acomp_cidadaos_vinculados.parquet", columns=coluns_acv
    )
    ...
    vac = pl.read_parquet(
        input_path + os.sep + "tb_fat_vacinacao.parquet", columns=coluns_vac
    )

    columns_proced = ["co_seq_fat_proced_atend", "co_fat_cidadao_pec", "co_dim_tempo"]

    proced_atend = pl.read_parquet(
        input_path + os.sep + "tb_fat_proced_atend.parquet", columns=columns_proced
    )

    fai_vars = ["co_fat_cidadao_pec", "co_dim_tempo"]

    fai = pl.read_parquet(
        input_path + os.sep + "tb_fat_atendimento_individual.parquet", columns=fai_vars
    )

    fao_vars = ["co_fat_cidadao_pec", "co_dim_tempo"]

    fao = pl.read_parquet(
        input_path + os.sep + "tb_fat_atendimento_odonto.parquet", columns=fao_vars
    )

    marc_cons_alim_vars = [
        "co_seq_fat_marca_con_almnt",
        "co_fat_cidadao_pec",
        "co_dim_tempo",
    ]

    marc_cons_alim = pl.read_parquet(
        input_path + os.sep + "tb_fat_marca_consumo_alimnt.parquet",
        columns=marc_cons_alim_vars,
    )
    ...
    atv_colet = pl.read_parquet(
        input_path + os.sep + "tb_fat_atvdd_coletiva_part.parquet",
        columns=atv_colet_vars,
    )

    selected_columns_cad = [
        "co_fat_cidadao_pec",
        "co_dim_tempo",
        "nu_uuid_ficha",
        "co_dim_raca_cor",
    ...
    fci = pl.read_parquet(
        input_path + os.sep + "tb_fat_cad_individual.parquet",
        columns=selected_columns_cad,
    )
    fci = fci.rename({"co_dim_tempo": "co_dim_tempo_fci"})

    tb_dim_equipe = pl.read_parquet(input_path + os.sep + "tb_dim_equipe.parquet")

    tb_dim_und_saude = pl.read_parquet(
        input_path + os.sep + "tb_dim_unidade_saude.parquet"
    )
    ...
    tb_pessoa_v2 = tb_pessoa.join(
        fci, left_on="co_unico_ultima_ficha", right_on="nu_uuid_ficha", how="left"
    )
    ...
    tb_pessoa_v3 = (
        tb_pessoa_v2.with_columns(
            pl.col("co_dim_tempo_fci")
            .cast(pl.Utf8)
            .str.strptime(pl.Date, format="%Y%m%d")
            .alias("dt_update_fci"),
            pl.when((pl.col("st_possui_fci") == 1) & (pl.col("st_possui_fcdt") == 1))
            .then(pl.lit("cadastro_completo"))  # cadastro completo
            .when((pl.col("st_possui_fci") == 1) & (pl.col("st_possui_fcdt") == 0))
            .then(pl.lit("cadastro_incompleto"))  # cadastro incompleto
            .when((pl.col("st_possui_fci") == 0) & (pl.col("st_possui_fcdt") == 0))
            .then(
                pl.lit("pessoa_ident_nao_cadastrada")
            )  # pessoa identificada e não cadastrada
            .when((pl.col("st_possui_fci") == 0) & (pl.col("st_possui_fcdt") == 1))
            ...
            pl.col("dt_atualizacao_fcd")
            .str.strptime(pl.Date, "%Y-%m-%d")
            .alias("dt_atualizacao_fcd"),
        )
        .with_columns(
            pl.when(
            ...
            pl.when(
                (
                    pl.col("dt_atualizacao_fcd") >= dt_24meses
                )  # (pl.col("status_cadastro") >= 0) &
            )
            .then(1)
            .otherwise(0)
            .alias("fcdt_att_2anos"),
            pl.when((pl.col("status_cadastro") != "cadastro_completo"))
            .then(1)
            .otherwise(0)
            .alias("alerta_status_cadastro"),
            pl.when(
                (pl.col("nu_cpf_cidadao").is_null())
                & (pl.col("nu_cns_cidadao").is_null())
            )
            .then(0)
            .otherwise(1)
        ...
        .with_columns(
            pl.when(
                (
                    pl.col("dt_update_fci") >= dt_24meses
                )  # (pl.col("status_cadastro") >= 0) &
            )
            .then(1)
            .otherwise(0)
            .alias("fci_att_2anos"),
            pl.when((pl.col("idade") >= 1) & (pl.col("idade") <= 4))
            .then(pl.lit("1a4"))
    ...
    procedimento_grouped = (
        procedimento.group_by("co_fat_cidadao_pec")
        .agg(pl.len().alias("n_procedimentos"))
        .with_columns(
            pl.when((pl.col("n_procedimentos") >= 1))
            .then(1)
            .otherwise(0)
            .alias("procedimento"),
        )
    ...
    atendimento = pl.concat([fai_v2, fao_v2, marc_cons_alim_v2, atv_colet_v2])

    atendimento_grouped = (
        atendimento.group_by("co_fat_cidadao_pec")
        .agg(pl.len().alias("n_atendimentos"))
        .with_columns(
            pl.when((pl.col("n_atendimentos") >= 1))
            .then(1)
            .otherwise(0)
            .alias("atendimentos"),
        )
    ...
    tb_pessoa_v4 = tb_pessoa_v3.join(
        atendimento_grouped, on="co_fat_cidadao_pec", how="left"
    )
    ...
    tb_pessoa_v6 = tb_pessoa_v5.join(
        tb_dim_equipe_v2, left_on="nu_ine_vinc_equipe", right_on="nu_ine", how="left"
    ).join(
        tb_dim_und_saude_v3,
    ...
    tb_pessoa_v7 = (
        tb_pessoa_v6.with_columns(
            pl.when(((pl.col("procedimento") == 1) & (pl.col("atendimentos") == 1)))
            .then(1)
            ...
            pl.when(
                (
                    (pl.col("fci_att_2anos") == 0)
                    | (pl.col("alerta_status_cadastro") == 1)
                    | (pl.col("fcdt_att_2anos") == 0)
                )
            )
            .then(1)
            .otherwise(0)
    ...
    tb_pessoa_v7.write_parquet(str(output_path+os.sep+"cadastro_db.parquet"))
    with DBConnectionHandler() as con:

        engine = con.get_engine()
        tb_pessoa_v7.with_columns(
            # Convert `co_dim_tempo` to datetime and create `dt_atendimento`
            pl.col("dt_nasc_cidadao").cast(pl.Datetime).dt.convert_time_zone("UTC"),
            pl.col("dt_update_fci").cast(pl.Datetime).dt.convert_time_zone("UTC"),
        ).with_columns(
            pl.col("co_fat_cidadao_pec").alias("cidadao_pec"),
            pl.col("co_cidadao").alias("co_cidadao"),
            pl.col("no_cidadao").alias("nome"),
            pl.col("co_dim_unidade_saude").alias("codigo_unidade_saude"),
            pl.col("codigo_equipe").alias("codigo_equipe_vinculada"),
            pl.col("dt_nasc_cidadao").alias("data_nascimento"),
            pl.col("nu_cpf_cidadao").alias("cpf"),
            pl.col("nu_cns_cidadao").alias("cns"),
            pl.col("acompanhamento").alias("acompanhamento"),
            pl.col("status_cadastro").alias("status_cadastro"),
            pl.col("idade").alias("idade"),
            pl.col("no_sexo_cidadao").alias("sexo"),
            pl.col("dt_update_fci").alias("ultima_atualizacao_cidadao"),
            pl.col("tipo_ident_cpf_cns").alias("tipo_ident_cpf_cns"),
            pl.col("faixa_etaria").alias("faixa_etaria"),
            pl.col("ds_raca_cor").alias("raca_cor"),
            pl.col("no_tipo_logradouro_tb_cidadao").alias("tipo_endereco"),
            pl.col("ds_logradouro_tb_cidadao").alias("endereco"),
            pl.col("nu_numero_tb_cidadao").alias("numero"),
            pl.col("no_bairro_tb_cidadao").alias("bairro"),
            pl.col("ds_complemento_tb_cidadao").alias("complemento"),
            pl.col("ds_cep_tb_cidadao").alias("cep"),
            pl.col("telefone").alias("telefone"),
            pl.col("nu_micro_area_domicilio").alias("nu_micro_area_domicilio"),
            pl.col("nome_equipe").alias("nome_equipe"),
            pl.col("nome_unidade_saude").alias("nome_unidade_saude"),
            pl.col("fci_att_2anos").alias("fci_att_2anos"),
            pl.col("fcdt_att_2anos").alias("fcdt_att_2anos"),
            pl.col("alerta_status_cadastro").alias("alerta_status_cadastro"),
            pl.col("alerta").alias("alerta"),
            pl.col("ds_tipo_localizacao_domicilio").alias("tipo_localidade"),
        ).select(
            "cidadao_pec",
            "co_cidadao",
            "nome",
            "codigo_unidade_saude",
            "codigo_equipe_vinculada",
            "data_nascimento",
            "cpf",
            "cns",
            "idade",
            "sexo",
            "ultima_atualizacao_cidadao",
            "tipo_ident_cpf_cns",
            "faixa_etaria",
            "raca_cor",
            "tipo_endereco",
