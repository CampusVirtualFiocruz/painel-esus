...
hypertension_cids = ",".join([ f"'{cid}'" for cid in hypertension.target])
LISTA_NOMINAL_HIPERTENSAO = f"""
with 
autoreferidos as (
...
creatinina as (
	select distinct on (co_fat_cidadao_pec) * from (
		select 
			co_fat_cidadao_pec , max(co_dim_tempo::text::date) as ultima_data_creatinina,
