...
def mergeMestre(gestantes, mestre):
    gestantes['co_fat_cidadao_pec'] = gestantes['co_fat_cidadao_pec'].astype(
        str).astype(float).astype(int)
    mestre['co_fat_cidadao_pec'] = mestre['co_fat_cidadao_pec'].astype(
        str).astype(float).astype(int)
    # gestantes['co_fat_cidadao_pec'] = gestantes['co_fat_cidadao_pec'].str[:-2]
    mestre = mestre[['co_fat_cidadao_pec', 'ds_tipo_localizacao']]
    gestante = gestantes.merge(
        mestre, on='co_fat_cidadao_pec', how='left', indicator=True)
...
def criarBaseFinalGestante(path):
    gestante = pd.read_csv(path + '/BASE_GESTACOES_3_INDICADORES.csv')
    gestante['co_fat_cidadao_pec_x'] = gestante['co_fat_cidadao_pec_x'].astype(
        str).str[:-2]
    ...
    gestante_SELECTED['dt_nascimento'] = pd.to_datetime(
        gestante_SELECTED['dt_nascimento'], format='%Y-%m-%d', errors='coerce')
    gestante_SELECTED['dt_nascimento'] = gestante_SELECTED['dt_nascimento'].where(
        gestante_SELECTED['dt_nascimento'] < now, gestante_SELECTED['dt_nascimento'] - np.timedelta64(100, 'Y'))
    gestante_SELECTED['AGE'] = (
        now - gestante_SELECTED['dt_nascimento']).dt.days.astype('int')
    ...
    gestante_SELECTED['FAIXA_ETARIA'] = gestante_SELECTED.apply(
        lambda row: age_group(row), axis=1)

    """**Calcular Idade gestacional**"""

    gestante_SELECTED['NEW_DUM_y'] = pd.to_datetime(
        gestante_SELECTED['NEW_DUM_y'], format='%Y-%m-%d', errors='coerce')
    gestante_SELECTED['co_dim_tempo'] = pd.to_datetime(
        gestante_SELECTED['co_dim_tempo'], format='%Y-%m-%d', errors='coerce')
    gestante_SELECTED['Difference'] = (
        gestante_SELECTED['co_dim_tempo'] - gestante_SELECTED['NEW_DUM_y']).dt.days
    ...
    gestante_SELECTED['IG'] = gestante_SELECTED.apply(
    ...
    gestante_SELECTED['nu_idade_gestacional'] = gestante_SELECTED.apply(
        lambda row: idade_gestacional(row), axis=1)

    """### Delete unecessary Column and Rename"""

    gestante_SELECTED_FINAL = gestante_SELECTED.drop(
        columns=['co_dim_equipe_1', 'nu_idade_gestacional_semanas', 'nu_gestas_previas', 'Difference', 'semanas_diff'])
    ...
    gestante_SELECTED_FINAL['co_dim_unidade_saude'] = gestante_SELECTED_FINAL['co_dim_unidade_saude'].replace(
        np.nan, 0)
    gestante_SELECTED_FINAL['co_dim_unidade_saude'] = gestante_SELECTED_FINAL['co_dim_unidade_saude'].astype(
        int)

    gestante_SELECTED_FINAL = gestante_SELECTED_FINAL.merge(
    ...
    gestante_SELECTED_FINAL['isContained_DIABETE'] = gestante_SELECTED_FINAL.astype(
    ...
    gestante_SELECTED_FINAL['isContained_HIPERTENSAO'] = gestante_SELECTED_FINAL.astype(
        str).apply(string_finder, words=match, axis=1)
    ...
    cadastro_mestre_ = cadastro_mestre[[
        'ds_tipo_localizacao', 'co_fat_cidadao_pec']]
    gestante_SELECTED_FINAL = mergeMestre(
        gestante_SELECTED_FINAL, cadastro_mestre_)
    gestante_SELECTED_FINAL.to_csv(
        path + '/BASE_GESTACOES_FINAL_PAINEL.csv')
    gc.collect()
