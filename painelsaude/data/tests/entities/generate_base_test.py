...
def test_segmentacao_base():
    ...
    for i in range(len(base.feminino)):    
        ...
        for j in range(10):        
            dt_nascimento = birthday_factory(min_age=faixa[0], max_age=faixa[1])
            idade = relativedelta(
...
def test_geracao():
    ...
    for pessoa in pessoas:
        ...
        p = pessoa.to_dict()
        p= parse_pessoa_cadatro_mestre(pessoa_dict=p, atendimentos=pessoa.atendimentos)
        pessoas_dict.append(p)
        
    df = pd.json_normalize(pessoas_dict)
    
    df.to_excel('../../../files/CADASTRO_MESTRE_JOIN_AT_IND_GEST_HIP_DIAB_JOIN_UNIDADES_CLEAN_PAINEL.xlsx', index=False)
    ...
    for pessoa in pessoas_hipertensao:
        ...
        set_data_arterial_hipertension_procedures(pessoa=pessoa, number_cases=number_cases)
    ...
    df.to_excel('../../../files/BASE_ATENDIMENTOS_X_HIPERTENSAO.xlsx', index=False)
    ...
    for pessoa in pessoas_diabetes:
        ...
        pessoa_dict = pessoa.to_dict()
        pessoa_dict = parse_pessoa_atendimento_model(pessoa_dict=pessoa_dict, atendimentos=pessoa.atendimentos, type="diabetes")
